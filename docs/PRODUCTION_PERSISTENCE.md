...
| Upload/index artifacts | Local filesystem, optionally B2 | B2 is required for workers on separate hosts |
...
Index mutations are serialized per user with a Redis lock (or a local lock in development), preventing concurrent workers from overwriting the same FAISS/BM25 artifacts.
...
`render.yaml` declares the web-service variables but does not silently create a paid background worker or managed databases. Add the PostgreSQL/Redis resources and worker explicitly in Render, then apply the deployment order above. If the plan supports a pre-deploy command, use `cd backend && alembic upgrade head`; otherwise run it from a one-off shell before switching `DATABASE_URL` on.
